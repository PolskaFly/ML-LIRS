...
def find_LIR(S: OrderedDict, pg: deque):
    while list(S.items())[0][2]:
        pg[list(S.items())[0]][3] = False
...
free_mem = MAX_MEMORY
print(pg_table)
lir_stack[1] = pg_table[1]
print(lir_stack)
# lir_stack[1] = 4
# lir_stack[2] = 7
# lir_stack[3] = 8
# print(list(lir_stack.items())[-1])
# lir_stack.move_to_end(1)
...
for i in range(len(trace)):
    ...
    if pg_table[ref_block][1]:
        ...
        if not pg_table[ref_block][2] and pg_table[3]:
            lir_stack.move_to_end(ref_block)
            find_LIR(lir_stack, pg_table)
        elif pg_table[ref_block][2] and pg_table[3]:
        ...
        PG_FAULTS += 1
        if free_mem == 0:
            set_residence(pg_table, list(hir_stack.items())[0], False)
            hir_stack.popitem(last=False)
            if pg_table[ref_block][3] and pg_table[2]:
                set_recency(pg_table, list(lir_stack.keys())[0], False)
                set_hir_block(pg_table, list(lir_stack.keys())[0], True)
                hir_stack[list(lir_stack.keys())[0]] = pg_table[list(lir_stack.keys())[0]]
                lir_stack.move_to_end(list(lir_stack.keys())[0])
                ...
                find_LIR(lir_stack, pg_table)
            elif not pg_table[ref_block][3] and pg_table[2]:
